...
import BudgetLineInput from '@/components/dialogs/BudgetLineInput';
...
const BudgetItemSection = ({ title, itemType, items, updateItem, addNewItem, removeItem, products, services, collaborators, focusOnNewItem, onNewItemFocused, showCosts, readOnly }) => {
    ...
    const handleSelect = (client_id, selectedItem) => {
        ...
        // Issue 4: Ensure we use the clean label provided by the component
    ...
    const handleKeyDown = (e, client_id) => {
        ...
        if (e.key === 'Enter') {
            ...
            const row = e.currentTarget ? e.currentTarget.closest('.budget-item-row') : 
                        (e.target ? e.target.closest('.budget-item-row') : null);
            ...
            // For BudgetLineInput, e.target is the input. For Select, it's the trigger button.
            ...
            {items.map((item, index) => (
                ...
                >
                    <BudgetLineInput 
                        ...
                        // Pass the enter handler. BudgetLineInput will call this when free text is entered.
